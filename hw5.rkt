...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
               ...
               (#t (error "int values required for ifgreater"))))]
        [(mlet? e)
        ...
        [(fun? e) (closure (cons (cons "form" e) env) (fun-body e))]
         ;fun (nameopt formal body)
        [(call? e)
         (let ([funexp (eval-under-env (call-funexp e) env)]
               [actual (eval-under-env (call-actual e) env)])
           (if (closure? funexp)
               (eval-under-env (closure-fun funexp) (let ([exp (envlookup (closure-env funexp) "form")])
                                                      (cons 
                                                       (cons (fun-formal exp) actual)
                                                       (if (fun-nameopt exp) (cons (cons (fun-nameopt exp) funexp) (closure-env funexp))
                                                                          (closure-env funexp)))))
               (error "not a closure")))]
         ;call (funexp actual)
         ;closure (env fun)
...
(define (ifeq e1 e2 e3 e4) 
  (mlet* (cons (cons "_x" e1) (cons (cons "_y" e2) null))
    (ifgreater (var "_x") (var "_y") e4 (ifgreater (var "_y") (var "_x") e4 e3))))

;; Problem 4

(define mupl-map 
  (fun #f "mf" (fun "mumap" "ms" 
                    (ifaunit (var "ms") (aunit)
                     (apair (call (var "mf") (fst (var "ms"))) (call (var "mumap") (snd (var "ms"))))))))
